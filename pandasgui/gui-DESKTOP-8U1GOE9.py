...
from widgets.find_toolbar import Find_Toolbar
...
class PandasGUI(QtWidgets.QMainWindow):
    ...
    def show_find_bar(self):
        if self.findBar.height() == 0:
            ...
            showAnimation = QtCore.QVariantAnimation(self.findBar)
            ...
            showAnimation.valueChanged.connect(lambda val: self.findBar.setFixedHeight(val))

            showAnimation.start()

    def hide_find_bar(self):
        if self.findBar.height() == 30:
            hideAnimation = QtCore.QVariantAnimation(self.findBar)
            hideAnimation.setEasingCurve(QtCore.QEasingCurve.InOutQuad)
            hideAnimation.setDuration(200)
            hideAnimation.setStartValue(30)
            hideAnimation.setEndValue(0)
            hideAnimation.valueChanged.connect(lambda val: self.findBar.setFixedHeight(val))
    ...
    def find(self, text):
        ...
        for i in range(model.columnCount()):
            ...
            matches = model.match(start, QtCore.Qt.DisplayRole,
                                  text, -1, QtCore.Qt.MatchFlags(reduce(lambda x, y: x| y, self.match_flags)))
    ...
    #     self.find_textbox = QtWidgets.QLineEdit()
    #     self.find_textbox = ButtonLineEdit(os.path.join(self.images_folder, 'case-match.png'))
    ...
    #     self.find_textbox.button.clicked.connect(self.toggle_match_case)
    #     find_replace_layout.addWidget(self.find_textbox)
    ...
    #     find_replace_layout.addWidget(close_find_button)

    #     self.findBar.addWidget(find_replace_widget)

    #     self.findBar.setFixedHeight(0)
