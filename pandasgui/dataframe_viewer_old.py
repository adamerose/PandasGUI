...
class HierarchicalHeaderView(QHeaderView):
    ...
    def on_sectionResized(self, logicalIndex: int):
        ...
        leafIndex = QModelIndex(self._pd.leafIndex(logicalIndex))
        if leafIndex.isValid():
            leafsList = list(self._pd.leafs(self._pd.findRootIndex(leafIndex)))
            try:
                listIndex = leafsList.index(leafIndex)
            except ValueError:
                listIndex = -1

            for n in range(listIndex, 0, -1):
                logicalIndex -= 1
    ...
    class private_data:
        ...
        def initFromNewModel(self, orientation: int, model: QAbstractItemModel):
            self.headerModel = model.makeHeaderModel(HorizontalHeaderDataRole if orientation == Qt.Horizontal else VerticalHeaderDataRole)
        ...
        def parentIndexes(self, index: QModelIndex) -> list:
            indexes = []
            while index.isValid():
        ...
        def searchLeafs(self, currentIndex: QModelIndex) -> list:
            res = []
            if currentIndex.isValid():
                childCount = currentIndex.model().columnCount(currentIndex)
                if childCount:
        ...
        def leafs(self, searchedIndex: QModelIndex) -> list:
            leafs = []
        ...
        def currentCellWidth(self, searchedIndex: QModelIndex, leafIndex: QModelIndex, sectionIndex: int,
                             hv: QHeaderView) -> int:
            leafsList = list(self.leafs(searchedIndex))
            if len(leafsList)==0:
            ...
            firstLeafSectionIndex = sectionIndex - listIndex
        ...
        def paintHorizontalSection(self, painter: QPainter, sectionRect: QRect,
            ...
            top = sectionRect.y()
            indexes = list(self.parentIndexes(leafIndex))
...
class DataFrameModel(QtCore.QAbstractTableModel):
    ...
    def headerData(self, section, orientation, role):
        # This isn't used, the headers use the data() function instead
        return None
...
class DataFrameView(QtWidgets.QTableView):
    ...
    def fixSize(self):
        ...
        # Disabling this because it is very slow for large tables
        # self.resizeColumnsToContents()

        # Fit column width to header text
        cols = self.model().columnCount(None)

        for i in range(cols):
            text = self.model().df.columns[i]
            label = QtWidgets.QLabel(str(text))
            width = label.fontMetrics().boundingRect(label.text()).width()*1.5
    ...
    def sizeHint(self):
        # Calculate width of rows and columns
        cols = self.model().columnCount(None)
...
def main():
    ...
    if test_case == 1:
        ...
        tuples = [('A', 'one', 'Xxxddddddddddddddx'), ('A', 'one', 'Y'), ('A', 'two', 'X'), ('A', 'two', 'Y'),
