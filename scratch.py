...
class DataFrameModel(QtCore.QAbstractTableModel):
    ...
    def readLevel(self, y=0, start=0, end=None, orient=None):
        ...
        for i in range(start, end):
            # Loop from start to end until a new label appears in this header level.
            label = cols.labels[y][i]
            if label != prev_label:
                if i < start:  # this condition was included below, but i dont think it should ever happen
                    sys.exit(4)
                # If there are more header levels below this one, each label should have the labels under it as children
                if y < len(cols.levels) - 1:  # and i > start: FIXME: this isn't needed, is it?
